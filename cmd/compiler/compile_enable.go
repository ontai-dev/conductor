...
// Patches the coredns ConfigMap (Talos CoreDNS name) in kube-system by adding a
// file plugin stanza for seam.ontave.dev with a 10s reload interval.
...
func writeCoreDNSDSNSStanzaYAML(dir string) error {
	cm := corev1.ConfigMap{
		...
		Data: map[string]string{
			// dsns-stanza is the block to append to the Corefile.
	...
	buf.WriteString("# Patches the coredns ConfigMap in kube-system (Talos CoreDNS name).\n")
	buf.WriteString("# Adds file plugin stanza for seam.ontave.dev with 10s reload.\n")
