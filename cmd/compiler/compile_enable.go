...
func conductorOp(version, registry, clusterName, clusterRole string) operatorSpec {
...
func allOperators(version, registry, clusterName, dsnsIP string) []operatorSpec {
	...
	cdt := conductorOp(version, registry, clusterName, "management")
...
func compileEnableBundle(output, version, registry, kubeconfig string, withCAPI bool, clusterName, dsnsIP, clusterRole string) error {
	...
	cdt := conductorOp(version, registry, clusterName, clusterRole)
...
func writePhase1GuardianBootstrap(output string, gdn operatorSpec) error {
	...
	// management-policy governs all seam operator RBACProfiles. Layer 1 fleet-wide policy.
...
// Emits the management-policy RBACPolicy CR that governs all seam operator
// RBACProfiles. Layer 1 fleet-wide governing policy (guardian-schema.md §19 Layer 1).
...
func writeBootstrapRBACPolicy(dir string) error {
	policy := map[string]interface{}{
		...
		"spec": map[string]interface{}{
			...
			"maximumPermissionSetRef": "management-maximum",
		},
	}
	...
	var buf bytes.Buffer
	buf.WriteString("# Bootstrap RBACPolicy -- management-policy\n")
	buf.WriteString("# Generated by: compiler enable (phase 1 guardian-bootstrap)\n")
	buf.WriteString("# Layer 1 fleet-wide governing policy (guardian-schema.md §19 Layer 1).\n")
	buf.WriteString("# Governs all seam operator RBACProfiles (guardian, platform, wrapper, conductor, seam-core).\n")
	buf.WriteString("# enforcementMode=audit: Guardian records violations but does not block during bootstrap.\n")
	buf.WriteString("# guardian-schema.md §6, §19.\n")
	buf.WriteString("---\n")
	buf.Write(data)
...
func writeBootstrapPermissionSets(dir string) error {
	...
	sets := []permSet{
		{
			...
			description: "Fleet permission ceiling -- Layer 1 of three-layer RBAC hierarchy",
			permissions: []map[string]interface{}{
	...
	for _, s := range sets {
		...
		if err != nil {
			return fmt.Errorf("marshal PermissionSet %s: %w", s.name, err)
		}
		buf.WriteString("---\n")
		buf.Write(data)
	}
...
func writePhase3PlatformWrapper(output string, ops []operatorSpec) error {
	...
	// platform-executor-role.yaml — Role and RoleBinding in ont-system for the
	// platform-executor SA. Conductor executor Jobs write OperationResult ConfigMaps
	// to their pod namespace (ont-system). CP-INV-010, conductor-schema.md §5.
	if err := writePlatformExecutorRoleFile(dir); err != nil {
		return err
	}
...
// conductor-schema.md §8, CP-INV-010.
func writePlatformExecutorRoleFile(dir string) error {
	role := rbacv1.Role{
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{"ontai.dev/rbac-owner": "guardian"},
		},
	...
	rb := rbacv1.RoleBinding{
		...
		Subjects: []rbacv1.Subject{
			{Kind: "ServiceAccount", Name: "platform-executor", Namespace: "ont-system"},
		},
	...
	if err != nil {
		return fmt.Errorf("marshal platform-executor Role: %w", err)
	}
	rbData, err := yaml.Marshal(rb)
	if err != nil {
		return fmt.Errorf("marshal platform-executor RoleBinding: %w", err)
	}

	var buf bytes.Buffer
	buf.WriteString("# platform-executor Role and RoleBinding in ont-system\n")
	...
	buf.WriteString("---\n")
	buf.Write(rbData)
	return os.WriteFile(filepath.Join(dir, "platform-executor-role.yaml"), buf.Bytes(), 0644)
}
...
func writePlatformWrapperCRDs(dir string) error {
	...
	// Filter to platform and infrastructure (seam-core owns all wrapper and conductor CRDs).
	groups := []string{"platform.ontai.dev", "infrastructure.ontai.dev"}
	...
	var buf bytes.Buffer
	buf.WriteString("# Platform and seam-core CRD Definitions\n")
	buf.WriteString("# Generated by: compiler enable (phase 3 platform-wrapper)\n")
...
// writeConductorCRDs writes the conductor-crds placeholder.
// InfrastructureRunnerConfig CRD is now owned by seam-core and included in
// the platform-wrapper-crds.yaml written in phase 3. This file is retained
// to preserve the phase 4 directory layout; it carries only a comment header.
func writeConductorCRDs(dir string) error {
	var buf bytes.Buffer
	buf.WriteString("# Conductor CRD Definitions\n")
...
func operatorClusterRules(operatorName string) []rbacv1.PolicyRule {
	...
	case "conductor":
		return append(common, rbacv1.PolicyRule{
			APIGroups: []string{"infrastructure.ontai.dev"},
			Resources: []string{"infrastructurerunnerconfigs", "infrastructurerunnerconfigs/status"},
	...
	case "guardian":
		return append(common,
			...
			// infrastructure.ontai.dev — Guardian reads InfrastructureRunnerConfigs in
			// ont-system to validate Conductor is operational before advancing bootstrap state.
	...
	case "wrapper":
		return append(common, rbacv1.PolicyRule{
			APIGroups: []string{"infrastructure.ontai.dev"},
			Resources: []string{"infrastructureclusterpacks", "infrastructurepackexecutions", "infrastructurepackinstances",
				"infrastructureclusterpacks/status", "infrastructurepackexecutions/status", "infrastructurepackinstances/status"},
			Verbs: []string{"get", "list", "watch", "create", "update", "patch", "delete"},
		})
	case "seam-core":
		return append(common, rbacv1.PolicyRule{
...
func writeWrapperRunnerRBACYAML(dir, clusterName string) error {
	...
	role := rbacv1.Role{
		...
		Rules: []rbacv1.PolicyRule{
			...
			{
				APIGroups: []string{"infrastructure.ontai.dev"},
				Resources: []string{"infrastructurepackexecutions", "infrastructureclusterpacks", "infrastructurepackinstances"},
			...
			},
			{
				...
				// delete is required to supersede previous revisions (single-active-revision
				// pattern T-15). infrastructure.ontai.dev/v1alpha1, seam-core PR #11.
	...
	// ClusterRole covering cluster-scoped non-RBAC resources applied by the
	// pack-deploy Job after guardian intake (bucket 2 of three-bucket split).
	...
	cr := rbacv1.ClusterRole{
		...
		ObjectMeta: metav1.ObjectMeta{
			Name: "wrapper-runner-cluster-scoped",
			Labels: map[string]string{
				"app.kubernetes.io/name":      "wrapper",
				"app.kubernetes.io/component": "runner",
				"ontai.dev/managed-by":        "compiler",
	...
	crb := rbacv1.ClusterRoleBinding{
		...
		ObjectMeta: metav1.ObjectMeta{
			Name: "wrapper-runner-cluster-scoped-" + clusterName,
			Labels: map[string]string{
				"app.kubernetes.io/name":      "wrapper",
				"app.kubernetes.io/component": "runner",
				"ontai.dev/managed-by":        "compiler",
	...
	crData, err := yaml.Marshal(cr)
	if err != nil {
		return fmt.Errorf("marshal wrapper-runner-cluster-scoped ClusterRole: %w", err)
	}
	crbData, err := yaml.Marshal(crb)
	if err != nil {
		return fmt.Errorf("marshal wrapper-runner-cluster-scoped ClusterRoleBinding: %w", err)
	}
