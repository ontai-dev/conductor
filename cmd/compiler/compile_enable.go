...
func writePhase5PostBootstrap(output string, operators []operatorSpec, dsnsIP, clusterName string) error {
	...
	meta := phaseMeta{
		...
		ReadinessGate: "No further readiness gate. The management cluster is fully " +
			...
			"MANUAL STEP REQUIRED: run coredns-dsns-patch.sh to append the DSNS " +
			"stanza to the Corefile key in the coredns ConfigMap.",
		ApplyOrder: files,
	}
	if err := writePhaseMeta(dir, meta); err != nil {
		return err
	...
	if err := writeCoreDNSDSNSPatchScript(dir); err != nil {
		return err
	}
...
// writeCoreDNSDSNSPatchScript writes coredns-dsns-patch.sh to the phase directory.
//
// CoreDNS reads only the "Corefile" key of the coredns ConfigMap in kube-system.
// A separate ConfigMap key (e.g. "Corefile.seam-addendum") is silently ignored.
// The only safe way to add the DSNS stanza is to read the current Corefile value,
// append the seam.ontave.dev zone block, and patch the ConfigMap back in place.
//
// The script uses kubectl + jq to perform this operation atomically. It must be
// run by the operator after apply of coredns-deployment-patch.yaml and before
// DSNS resolution is expected to be functional.
//
// File path /etc/coredns/dsns/zone.db matches the volume mount in
// coredns-deployment-patch.yaml. 10s reload keeps zone freshness low.
// seam-core-schema.md §8 Decision 3.
func writeCoreDNSDSNSPatchScript(dir string) error {
	var buf bytes.Buffer
	buf.WriteString("#!/usr/bin/env bash\n")
	buf.WriteString("# coredns-dsns-patch.sh — append DSNS stanza to CoreDNS Corefile.\n")
	buf.WriteString("# CoreDNS only reads the Corefile key; a separate key is silently ignored.\n")
	buf.WriteString("# Run once after applying coredns-deployment-patch.yaml.\n")
	buf.WriteString("# Requires: kubectl (context pointed at the target cluster), jq.\n")
	buf.WriteString("# Generated by: compiler enable — seam-core-schema.md §8 Decision 3.\n")
	buf.WriteString("set -euo pipefail\n\n")
	buf.WriteString("CURRENT=$(kubectl get cm coredns -n kube-system -o jsonpath='{.data.Corefile}')\n")
	buf.WriteString(`NEW="${CURRENT}
seam.ontave.dev. {
    file /etc/coredns/dsns/zone.db
    reload 10s
}"
`)
	buf.WriteString("jq -n --arg cfg \"$NEW\" '{\"data\": {\"Corefile\": $cfg}}' | \\\n")
	buf.WriteString("    kubectl patch cm coredns -n kube-system --type merge --patch-file /dev/stdin\n")
	return os.WriteFile(filepath.Join(dir, "coredns-dsns-patch.sh"), buf.Bytes(), 0755)
}
