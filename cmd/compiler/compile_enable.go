...
// writeCoreDNSDSNSStanzaYAML writes the CoreDNS DSNS stanza reference file.
// The file contains the stanza text that must be appended to the Corefile key
// inside the coredns ConfigMap in kube-system (Talos CoreDNS name is "coredns",
// not "kube-dns"). The operator must read the current Corefile, append the
// seam.ontave.dev zone block, and kubectl edit or kubectl patch the ConfigMap.
//
// The stanza block is stored in the file plugin ConfigMap key "Corefile.seam-addendum"
// as a reference. CoreDNS does NOT load this key automatically — the operator must
// merge it into the active Corefile key.
//
...
func writeCoreDNSDSNSStanzaYAML(dir string) error {
	cm := corev1.ConfigMap{
		TypeMeta: metav1.TypeMeta{APIVersion: "v1", Kind: "ConfigMap"},
		ObjectMeta: metav1.ObjectMeta{
			// Talos CoreDNS ConfigMap is named "coredns", not "kube-dns".
			Name:      "coredns",
			Namespace: "kube-system",
			Annotations: map[string]string{
				"governance.infrastructure.ontai.dev/owner": "seam-core",
		...
		Data: map[string]string{
			// Corefile.seam-addendum is the stanza to append to the active Corefile key.
			// CoreDNS does NOT load this key — it is a reference for the operator.
			// The operator must: kubectl edit configmap coredns -n kube-system
			// and append this block to the Corefile key before the final closing brace.
			// File path /etc/coredns/dsns/zone.db matches coredns-deployment-patch.yaml.
			"Corefile.seam-addendum": "seam.ontave.dev. {\n    file /etc/coredns/dsns/zone.db\n    reload 10s\n}\n",
		},
	}

	var buf bytes.Buffer
	buf.WriteString("# CoreDNS DSNS Stanza Reference — DO NOT apply directly.\n")
	buf.WriteString("# The Corefile.seam-addendum key contains the stanza block to append\n")
	buf.WriteString("# to the active Corefile in the coredns ConfigMap (kube-system).\n")
	buf.WriteString("# Operator procedure:\n")
	buf.WriteString("#   kubectl edit configmap coredns -n kube-system\n")
	buf.WriteString("#   Append the Corefile.seam-addendum value to the Corefile key.\n")
	buf.WriteString("# Generated by: compiler enable\n")
