...
// TestSnapshotPullLoop_FormalCRDSchema_SubjectsField verifies that a PermissionSnapshot
// constructed with the formal CRD spec.subjects field (guardian schema §7, commit 0fd9cd1)
// is correctly parsed by the pull loop and populates the SnapshotStore with one
// PrincipalPermissionEntry per SubjectEntry.
//
// This is the pre-lab compatibility test: it confirms that the pull loop can consume
// snapshots generated by guardian's EPGReconciler after the formal schema was adopted.
func TestSnapshotPullLoop_FormalCRDSchema_SubjectsField(t *testing.T) {
	...
	spec := map[string]interface{}{
		...
		"version":                now,
		"generatedAt":            now,
		"snapshotTimestamp":      now, // formal CRD field
		"signingKeyFingerprint":  "sha256:testfingerprint0000",
		"freshnessWindowSeconds": int64(300),
		"subjects": []interface{}{
			map[string]interface{}{
				"subjectName": "alice",
	...
	// Bootstrap window mode: no key mounted, so unsigned snapshot is accepted.
	snapshot := makeSnapshot("snap-formal-schema", "security-system", "", spec)
	store := permissionservice.NewSnapshotStore()
	loop := agent.NewSnapshotPullLoop(
		newMgmtClient(snapshot), newLocalClient(), store, "ccs-test", "ont-system",
	)
	...
	entries, version, cluster, ready := store.Get()
	if !ready {
		t.Fatal("expected store ready after formal-schema snapshot pull; not ready")
	}
	if cluster != "ccs-test" {
		t.Errorf("cluster: got %q; want ccs-test", cluster)
	}
	if version != now {
		t.Errorf("version: got %q; want %q", version, now)
	}
	// Two SubjectEntry values → two PrincipalPermissionEntry values.
	if len(entries) != 2 {
		t.Fatalf("expected 2 PrincipalPermissionEntry (one per SubjectEntry); got %d", len(entries))
	}

	// Verify alice entry.
	aliceIdx := -1
	for i, e := range entries {
		if e.PrincipalRef == "alice" {
			aliceIdx = i
			break
		}
	}
	if aliceIdx < 0 {
		t.Fatal("PrincipalRef 'alice' not found in store entries")
	}
	alice := entries[aliceIdx]
	// alice has 2 PermissionEntries: ("", [pods, services]) and ("apps", [deployments])
	// Expansion: ("", pods), ("", services), ("apps", deployments) → 3 AllowedOperations.
	if len(alice.AllowedOperations) != 3 {
		t.Errorf("alice: expected 3 AllowedOperations; got %d", len(alice.AllowedOperations))
	}

	// Verify bob entry.
	bobIdx := -1
	for i, e := range entries {
		if e.PrincipalRef == "bob" {
			bobIdx = i
			break
		}
	}
	if bobIdx < 0 {
		t.Fatal("PrincipalRef 'bob' not found in store entries")
	}
	bob := entries[bobIdx]
	// bob has 1 PermissionEntry: ("", [configmaps]) → 1 AllowedOperation.
	if len(bob.AllowedOperations) != 1 {
		t.Errorf("bob: expected 1 AllowedOperation; got %d", len(bob.AllowedOperations))
	}
	if bob.AllowedOperations[0].Resource != "configmaps" {
		t.Errorf("bob.AllowedOperations[0].Resource: got %q; want configmaps", bob.AllowedOperations[0].Resource)
	}
}
