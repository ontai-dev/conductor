...
func TestRunExecute_TwoSimultaneous_NoCrossContamination(t *testing.T) {
	steps1 := []runnerlib.RunnerConfigStep{
		{Name: "alpha", Capability: runnerlib.CapabilityBootstrap},
	}
	steps2 := []runnerlib.RunnerConfigStep{
		{Name: "beta", Capability: runnerlib.CapabilityPackDeploy},
	}

	writer1 := &recordingStepStatusWriter{}
	writer2 := &recordingStepStatusWriter{}
	exec1 := &fixedStepExecutor{defaultPhase: runnerlib.StepPhaseSucceeded}
	exec2 := &fixedStepExecutor{defaultPhase: runnerlib.StepPhaseSucceeded}
	...
	if writer1.stepResults[0].StepName != "alpha" {
		t.Errorf("writer1: expected StepName=%q; got %q", "alpha", writer1.stepResults[0].StepName)
	}
	...
	if writer2.stepResults[0].StepName != "beta" {
		t.Errorf("writer2: expected StepName=%q; got %q", "beta", writer2.stepResults[0].StepName)
	}
...
func TestRunExecute_PlatformOwnerRef_RoutingUnaffected(t *testing.T) {
	...
	if writer.stepResults[0].StepName != "platform-bootstrap" {
		t.Errorf("StepName: got %q; want %q", writer.stepResults[0].StepName, "platform-bootstrap")
	}
}
