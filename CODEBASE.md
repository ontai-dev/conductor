...
|------|-------------|--------------|
| `compile.go` | `PackBuildInput` (L498), `ClusterInput` (L382), `BootstrapSection` (L191), `compileBootstrap()`, `compilePackBuild()`, `readClusterInput()` | Entry points for bootstrap/packbuild; ClusterInput parsed from YAML; dispatch to helm or raw path. `readClusterInput` enforces the management bootstrap guard: `mode=bootstrap` with `role=""` or `role="management"` is rejected with an error directing the operator to use `mode=import` instead. Tenant clusters (`role=tenant`) may use `mode=bootstrap`. |
...
| `compile_oci_push.go` | `ociPushLayer()`, `pushBlob()`, `pushManifest()`, `resolveDockerAuth()` | OCI Distribution Spec v2 push helpers. `resolveDockerAuth(registry)` reads `~/.docker/config.json` and falls back to `~/snap/docker/current/.docker/config.json` (snap-installed Docker); returns the base64-encoded auth string for the registry or empty string. `pushBlob` and `pushManifest` accept an `authHeader` parameter and set `Authorization: Basic <auth>` when non-empty. Error messages include the full URL and a 401/403 auth hint for non-2xx responses. |
...
| `kubernetes_version_drift_loop.go` | `KubernetesVersionDriftLoop`, `NewKubernetesVersionDriftLoop()` | tenant | Reads `node.status.nodeInfo.kubeletVersion` from every node; strips leading `v`; compares against `InfrastructureTalosCluster.spec.kubernetesVersion` in `ont-system`. When all nodes agree on a consistent version that differs from spec, emits a DriftSignal named `drift-k8s-version-{cluster}` with `affectedCRRef.Kind=InfrastructureTalosCluster` and `driftReason="kubernetes version drift: spec={x} observed={y}"`. Mixed-version nodes (mid-upgrade) produce no signal. Platform's `DriftSignalReconciler` dispatches by driftReason prefix and creates a corrective `drift-k8s-version-{cluster}` UpgradePolicy with `upgradeType=kubernetes`. |
...
`role=tenant`: PermissionService gRPC server (port 50051), SnapshotPullLoop, PackInstancePullLoop, PackReceiptDriftLoop, RBACProfilePullLoop (conductor-tenant), RBACPolicyPullLoop (cluster-policy), TalosVersionDriftLoop, KubernetesVersionDriftLoop, FederationClient, AdmissionWebhook. Five pull loops + two drift loops fully wired. Decision C satisfied. T-17 closed. T-23 Talos and K8s version drift closed.

**`onLeaderStart()` signature**: clusterRef, namespace, manifest, publisher, reconciler, signingLoop, snapshotPullLoop, packInstancePullLoop, packReceiptDriftLoop, rbacProfilePullLoop, rbacPolicyPullLoop, driftSignalHandler, talosVersionDriftLoop, kubernetesVersionDriftLoop, dynamicClient.
...
| `kubeUpgradeHandler` | `kube-upgrade` | (`platform_upgrade.go`) Updates kubelet image on all nodes via full machine config merge. Reads `targetKubernetesVersion` (stored without `v` prefix) from UpgradePolicy CR. Builds kubelet image as `ghcr.io/siderolabs/kubelet:v{version}` (prepends `v`). For each node: calls `GetMachineConfig` to fetch the full current config, merges the kubelet image patch via `mergeYAMLPatch`, then calls `ApplyConfiguration` in `no-reboot` mode. Direct patch without GetMachineConfig fails -- Talos rejects partial configs missing required fields (`machine.ca`, cluster instructions). |
| `stackUpgradeHandler` | `stack-upgrade` | (`platform_upgrade.go`) Combines talos-upgrade (installer image, immediate reboot) and kube-upgrade (kubelet image, no-reboot) steps. Reads both `targetTalosVersion` and `targetKubernetesVersion` from UpgradePolicy CR. Kubelet step uses same GetMachineConfig+mergeYAMLPatch+ApplyConfiguration pattern with `staged` mode. Produces ≥2 StepResults. |
...
| Package | Coverage |
|---------|----------|
| `test/unit/agent` | PackInstancePullLoop, SnapshotPullLoop, DriftSignalHandler, PackReceiptDriftLoop (15 tests including `TestPackReceiptDriftLoop_NamespaceTerminating_SkipsDrift`), TalosVersionDriftLoop (4 tests: emits on version mismatch, skips when all nodes match spec, skips on mixed versions, ParseTalosVersionFromOSImage), KubernetesVersionDriftLoop (5 tests: `TestKubernetesVersionDriftLoop_EmitSignalOnDrift`, `TestKubernetesVersionDriftLoop_NoSignalWhenVersionsMatch`, `TestKubernetesVersionDriftLoop_NoSignalOnMixedVersions`, `TestKubernetesVersionDriftLoop_ConfirmSignalWhenResolved`, `TestKubernetesVersionDriftLoop_IncrementCounterOnQueued`) |
...
| `test/unit/capability` | Split path, guardian intake, RBAC apply, registry; hardeningApplyHandler (6 tests); talosUpgrade rolling (2 tests: `TestTalosUpgrade_RollingUpgrade_AllNodes` 3-node sequential with reboot wait, `TestTalosUpgrade_NoNodesReturnsValidationFailure`); kubeUpgrade (3 tests: `TestKubeUpgrade_AppliesKubeletConfig` verifies `v`-prefixed kubelet image in applied config, `TestKubeUpgrade_GetMachineConfigErrorReturnsFailure` guards GetMachineConfig path, nil-client validation); stackUpgrade (2 tests: `TestStackUpgrade_RunsBothUpgradeSteps` asserts talos Upgrade called and kubelet image has `v` prefix); pack-deploy handler (`wrapper_test.go`: nil-clients, missing OperationResultCM, fetch+apply manifests, OCI failure, `TestPackDeploy_MultiplePEsSameCluster_DeploysCorrectPack` -- guards against alphabetical-first-match bug) |
...
**Kubelet image `v` prefix in kube-upgrade**: `UpgradePolicy.spec.targetKubernetesVersion` follows the TalosCluster convention of no `v` prefix (e.g. `1.32.3`). The kubelet OCI image from `ghcr.io/siderolabs/kubelet` requires the `v` prefix (e.g. `v1.32.3`). `kubeUpgradeHandler` and the kube step in `stackUpgradeHandler` both prepend `v` in the format string. Do not store the version with `v` in UpgradePolicy, or the image reference becomes `vv{version}` and will not be found in the registry.

**Talos `ApplyConfiguration` rejects partial configs**: Sending a minimal JSON blob like `{"machine":{"kubelet":{"image":"..."}}}` directly to `TalosClient.ApplyConfiguration` fails with "cluster instructions are required" and "machine.ca required". The correct pattern is: call `GetMachineConfig` to fetch the full current node config, deep-merge the patch via `mergeYAMLPatch`, then call `ApplyConfiguration` with the merged bytes. This is the same pattern used by `hardeningApplyHandler` (`platform_security.go`).
