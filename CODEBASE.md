...
| `compile_packbuild_helm.go` | `HelmSource` struct (L33: URL, Chart, Version, ValuesFile, Namespace, RegistryCredentialsSecret), `helmCompilePackBuild(ctx, in, inputDir, output)` | Fetches chart, renders, splits RBAC/cluster-scoped/workload layers, pushes OCI, emits ClusterPack CR; writes ChartURL/ChartVersion/ChartName/HelmVersion at L252-255 |
| `compile_packbuild_raw.go` | `RawSource` struct (L24: Path), `rawCompilePackBuild(ctx, in, inputDir, output)` (L34) | Reads all .yaml/.yml from RawSource.Path, splits into 3 OCI layers, pushes, emits ClusterPack CR; chart fields NOT written (raw packs have no chart metadata by design) |

**`PackBuildInput` struct fields** (`compile.go:498`): Name, Namespace, Version, RegistryURL, Digest, Checksum, SourceBuildRef, TargetClusters, RBACDigest, ClusterScopedDigest, WorkloadDigest, BasePackName, ValuesFile, HelmSource `*HelmSource`, RawSource `*RawSource`. **No `Category` field** -- dispatch is by nil-check on HelmSource/RawSource pointers (T-05 open).
...
| `snapshot_pull_loop.go` | `SnapshotPullLoop`, `NewSnapshotPullLoop()` (L41), `NewSnapshotPullLoopWithKey()` (L58), `pullOnce()` (L107) | tenant | Pulls PermissionSnapshots from management cluster; verifies signatures; populates in-memory SnapshotStore for local gRPC PermissionService |
...
`role=tenant`: PermissionService gRPC server (L148-165, port 50051), SnapshotPullLoop (L190-205), PackInstancePullLoop (L209-224), PackReceiptDriftLoop (L230-235), FederationClient (L280-311), AdmissionWebhook.

**`onLeaderStart()` signature** (L378): clusterRef, namespace, manifest, publisher, reconciler, signingLoop, snapshotPullLoop, packInstancePullLoop, packReceiptDriftLoop, driftSignalHandler, dynamicClient. **RBACProfile pull loop is absent** -- no fourth pull loop for role=tenant exists (CONDUCTOR-BL-TENANT-ROLE-RBACPROFILE-DISTRIBUTION open, Decision C partially unfulfilled).
